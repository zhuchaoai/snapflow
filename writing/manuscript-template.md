...
标题（一行，20字以内，与 frontmatter 的 title 一致，两者都写；**正文标题正常用标点**，如「Syncthing一直走中继，一个操作转直连」）
...
# ── 封面断句与高亮规则（仅封面截图；正文标题正常用标点）──
# 完整权威规则见 cover-title-rules.md。速记：
# 1. 封面截图版：禁止 <br>；禁止标点；超宽用空格按语义切段
# 2. ★空格加在「这里 Slides 区 title/subtitle 值」；正文 # 标题不受限、正常用标点
# 3. 封面截图超宽无空格 → 引擎拦截不出图；高亮词用 <em> 包（内容勿用 **）
