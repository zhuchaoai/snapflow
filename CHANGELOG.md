...
### Added
- **数据层装饰字段（decor）**：`md2content` 为每张图生成确定性随机装饰参数（卡片角度/错位/装饰点坐标），同文件名可复现；数据层常驻 content.json，任何风格包可消费
- **正文关键词高亮**：`**词**`（Markdown 加粗）自动渲染为高亮标记，写稿零负担
- **卡片序号角标 + 随机错落布局**：风格包声明 `pack.layout=sticker` 即启用，其他风格包零影响

### Changed
- 封面标题断行优化：空格作为语义断点最高优先，修复带高亮标签标题的语义断行（019/020）
...
### Changed
- 稿件格式精简：移除已废弃的写作辅助字段及对应写稿约束，正文结构更简洁
- 头条改写套件：结构样板与映射表同步精简，与主稿件格式对齐
...
- `batch-screenshot.js`：ComfyUI 启动轮询调整 20s→25s→25s（开机后首次启动慢，原 10s 轮询不够）
- **封面铁律**：title/subtitle/tagline 禁止手写 `<br>`、禁止标点——引擎强制（短标题 ≤8 字不拆行 + 标点清洗兜底），写稿规范同步声明（003 样板副标题改金句，弃用"品牌+期号"惯例）
